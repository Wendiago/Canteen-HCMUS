    ...
    >
      {
        <Typography
          sx={{
            flex: "1 1 100%",
            color: "#EA7C69",
            fontWeight: "fontWeight.tableTitle",
...
const PreorderList = (props) => {
  ...
  const { preorderListRows, setPreorderListRows, setPreorderListOriginalRows } =
  ...
  const handleConfirmPopup = async () => {
    setSuccess(true);
    const confirmPayment = await orderApi.confirmPayment(
      { token, clientId },
      selected._id
    );
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    const deleteOrder = await orderApi.deleteOrder(
      { token, clientId },
      selected._id
    );
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - preorderListRows.length)
      : 0;
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Dialog open={openCard} onClose={handleCloseCard} maxWidth="md" fullWidth>
        <Card
          className="col-span-1 fixed right-6 top-2 h-screen w-1/4 p-4 rounded-lg"
          sx={{
            color: "white",
            minWidth: "400",
        ...
        >
          ...
          <div style={{ maxHeight: "calc(100vh - 320px)", overflowY: "auto" }}>
            {selectedRowData &&
              selectedRowData.map((selectedCard) => (
