...
const OrderList = (props) => {
  ...
  const { orderListRows, setOrderListRows, setOrderListOriginalRows } =
  ...
  const confirmCancel = async () => {
    ...
    setLoading(true);
    const res = await orderApi.cancelOrder({ token, clientId }, selected._id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - orderListRows.length) : 0;
